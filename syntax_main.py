...
def creat_map(proto_list, dissect_list):
    proto_dissect_map = {}
    for dissect in dissect_list:
        proto_name = dissect["proto_name"]
        func_name = dissect["func_name"]
        if proto_name in proto_dissect_map.keys():
            list1 = [proto_dissect_map[proto_name]]
            proto_dissect_map[proto_name] = list1
            proto_dissect_map[proto_name].append(func_name)
        else:
            proto_dissect_map[proto_name] = func_name
    return proto_dissect_map


# Hierarchical relation extraction
def extract_relation(lines, dissect_list, file_name):
    annot = False
    for line in lines:
        line = line.strip()
        if line.startswith('/*'):
        ...
        if line.endswith('*/'):
            ...
            continue
        if not annot:
            if line.startswith("heur_dissector_add"):
                content = extract_param(line)
                table_name = content[0]
                dissect_handle = content[1]
                dissect_description = content[2]
                dissect_name = content[3]
                proto_handle = content[4]
                heuristic_tag = content[5]
                for dissect in dissect_list:
                    if dissect["dissect_handle"] == dissect_handle:
                        dissect["lower"] = table_name            
                print()
            if line.startswith("dissector_add_uint_with_preference"):
                content = extract_param(line)
                table_name = content[0]
                port_num = content[1]
                dissect_handle = content[2]
                for dissect in dissect_list:
                    if dissect["dissect_handle"] == dissect_handle:
                        dissect["lower"] = table_name
            if "register_dissector_table" in line:
                content = extract_param(line)
                table_name = content[0]
                dissect_description = content[1]
                proto_handle = content[2]
                data_type = content[3]
                if "STRING" in data_type:
                    ...
                    BASE = content[4]
                for dissect in dissect_list:
                    if dissect["proto_handle"] == proto_handle:
                        dissect["register_table"] = table_name        
            if "register_heur_dissector_list" in line:
                content = extract_param(line)
                table_name = content[0]
                proto_handle = content[1]
                for dissect in dissect_list:
                    if dissect["proto_handle"] == proto_handle:
                        dissect["register_table"] = table_name
            if "find_dissector" in line:
                dissect_handle = line.split("=")[0].strip()
                content = extract_param(line)
                table_name = content[0]
                for dissect in dissect_list:
                    if dissect["file_name"] == file_name:
                        dissect["register_table"] = table_name
...
def remove_comments(source_file_path, output_file_path):
    try:
        with open(source_file_path, 'r', encoding='utf-8') as file:
            content = file.read()
        ...
        print(f"Content copied from '{source_file_path}' to '{output_file_path}'.")
...
# Extract protocol name and associated with file, save to dissect_list
...
# Pre-processing
def pre_process(directory, pre_process_dir, proto_files_dir):
    ...
    for root, dirs, files in os.walk(directory):
        for file_name in files:
            ...
            if file_name == ("packet-mbtcp.c") or file_name == ("packet-dccp.c") or file_name == ("packet-dnp.c") or file_name == ("packet-s7comm.c"):
                source_file_path = os.path.join(root, file_name)
                num += 1

                output_file_path = os.path.join(proto_files_dir, file_name)
                
                os.makedirs(os.path.dirname(output_file_path), exist_ok=True)

                remove_comments(source_file_path, output_file_path) 
                remove_conditional_comments(output_file_path)

                temp_no_condition_comments_file_path = os.path.join(pre_process_dir,'temp_no_condition_comments.c')
                with open(output_file_path, 'r', encoding='utf-8') as source:
                    with open(temp_no_condition_comments_file_path, 'w', encoding='utf-8') as destination:
                        for line in source:
                            destination.write(line)

                convert_to_single_line(output_file_path, "=")
                convert_to_multi_line(output_file_path, 'if')

                temp_preprocess_final_file_path = os.path.join(pre_process_dir,'temp_preprocess_final.c')
                with open(output_file_path, 'r', encoding='utf-8') as source:
                    with open(temp_preprocess_final_file_path, 'w', encoding='utf-8') as destination:
                        for line in source:
                            destination.write(line)
                ...
                with open(output_file_path, 'r', encoding = 'utf-8') as file:
                    ...
                    if register_code:
                        for proto in register_code:
                            ...
                            print("protocol name:", proto_names)
                            print("protocol handle:", proto_handle)
                    # global dissector
                    ...
                    # save dissector to dissect_list
                    for dissect in this_dissect:
                        dissect_list.append(dissect)
                    this_dissect = []

    # improve hierarchical relation
    improve_relation(dissect_list)

    # pre-processing time
    end_time = time.time()
    run_time = end_time - start_time
    print(f"pre-processing time:{run_time:.2f} 秒")
    print("protocol files number:", num)
    return proto_list, dissect_list, dissect_all

def select_proto(proto_list, dissect_list, proto_files_dir, proto_name, dissect_name, target_proto_info):
    lower_proto = False
    lower_proto_name = ''
    lower_dissect_name = ''
    func_name = ''
    ...
    for dissect in dissect_list:
        if dissect["dissect_name"] == dissect_name:
            ...
            if dissect['lower'] and (dissect['lower'] not in ('tcp', 'udp')):
                for low_dissect in dissect_list:
                    if ('dissect_handle' not in low_dissect) or ('proto_identifier_name' not in low_dissect):
                        continue
                    elif low_dissect['proto_identifier_name'] == dissect['lower']:
    ...
    if lower_proto:
        select_proto(proto_list, dissect_list, proto_files_dir, lower_proto_name, lower_dissect_name, target_proto_info)

    return target_proto_info

def save_field_to_dict(field_identifier, fieldstring):
    field_name = fieldstring.split(',')[0].strip("{ ").strip('"')
    field_abbreviation = fieldstring.split(',')[1].strip().strip('"')
    field_type = fieldstring.split(',')[2].strip()
    field_display_base = fieldstring.split(',')[3].strip()
    field_display_format = fieldstring.split(',')[4].strip()
    display_flags = fieldstring.split(',')[5].strip()
    representation = fieldstring.split(',')[6].strip()
    eng_tag = fieldstring.split(',')[7].strip().strip(" }")
    field_info = {
        "field_identifier": field_identifier,
        "field_name": field_name,
        "field_abbreviation": field_abbreviation,
        "field_type": field_type,
        "field_display_base": field_display_base,
        "field_display_format": field_display_format,
...
def add_statement(file_path, string):
...
def find_main_dissector1(lines, proto_dissector):
    main_dissector = proto_dissector
    in_function = False
    num = 0
    first_line = True
    for line in lines:
        ...
        if "}" in line:
            num -= 1
        if (line.startswith(proto_dissector + "(")) or (line.startswith("static gint " + proto_dissector + "(")):
            in_function = True
        elif in_function:
            first_line = False
            if (proto_dissector in line) or ('dissect_' in line):
                items = re.split(r'[,(\s)]', line)
                for item in items:
                    if "dissect_" in item:
                        main_dissector = find_main_dissector(lines, item)
...
def add_sub_func_body(lines, dissect_name, code, dissect_list, subfunction_call_format, dissect_func_name_list):
            ...
            if curr_dissect_name == dissect_name:
                ...
                for line in lines:
                    if line.startswith(dissect_name + "(") and ";" not in line:
                        add_sub_func_body(lines, dissect_name, code, dissect_list, subfunction_call_format, dissect_func_name_list)
...
def find_condition(if_stack):
    else_condition = ''
    for i in range(len(if_stack)-1,-1,-1):
        condition = if_stack[i]
        if condition[1] == 'if':
            else_condition = condition[2]
...
def delete_condition(if_stack):
    new_stack = []
    for i in range(len(if_stack)-1,-1,-1):
        condition = if_stack[i]
        if condition[1] == 'if':
            new_stack = if_stack[:i]
            return new_stack
...
def check_nested_sublists(packet_list):
    for sublist in packet_list:
        for subsublist in sublist:
            if subsublist and subsublist[0] in ( 'if', 'else if', 'else', 'while', 'for', 'do_while', 'case', 'default'):
                return False
    return True
...
def delete_return_packet(unique_packet_types):
    delete_return_packet = []
    for packet in unique_packet_types:
        delete_return = False
        delete_function_end = False
        ...
        for block in packet:
            if block[0] == 'return':
                ...
                break
            if block[0] == 'function':
                delete_function_end = True
                break
        
        if delete_return and not delete_function_end:
            new_packet = []
            first_part = []
            second_part = []
            for i in range(len(packet)):
                ...
                if block[0] == 'return':
                    first_part = packet[:i]
                    break
            for j in range(len(packet)-1, -1, -1):
                block = packet[j]
                if block[0] == 'function_end':
                    second_part = packet[j:]
                    break  
            new_packet.extend(first_part) 
            new_packet.extend(second_part)
            delete_return_packet.append(new_packet)
        elif delete_return and delete_function_end:
            new_packet = []
            first_part = []
            second_part = []
            ...
            new_packet.extend(second_part)
            delete_return_packet.append(new_packet)                
        elif not delete_return and delete_function_end:
            for i in range(len(packet)-1, -1, -1):
                block = packet[i]
                if block[0] == 'function_end':
                    packet.pop(i)
                    break
            delete_return_packet.append(packet)
        elif not delete_return and not delete_function_end:
            delete_return_packet.append(packet)
    return delete_return_packet
...
def delete_return_block(packet_types_expanded):
    packet_types_delete_return = []

    for packet in packet_types_expanded:
        for block in packet:
            if block[0] == 'return':
                print(block[0])

    return packet_types_delete_return


def is_offset_declaration(line):
    pattern = re.compile(r'\b(int|guint)\s+offset\s*;|\b(guint)\s+offset\s*=\s*\d+\s*;')
...
def merge_packet_types(packet_types_expanded_list):
    ...
    unique_packet(packet_types_merged)
...
# Main-parsing
def main_analyse(file_path, file_name, func_name, preprocess_file_path, main_parsing_dir, truth_field_dir, output_dir, dissect_list, dissect_name):
    packet_types = []
    ...
    with open(file_path, 'r') as file:
        ...
        all_field_dir = os.path.join(main_parsing_dir,'all_fields')
        if not os.path.exists(all_field_dir):
            os.makedirs(all_field_dir)
        all_field_path = os.path.join(all_field_dir, f"{os.path.splitext(file_name)[0]}.json")
        with open(all_field_path, 'w', encoding='utf-8') as file:
        ...
        without_multi_line_file_path = os.path.join(main_parsing_dir,'without_multi_line.c')
        with open(file_path, 'r', encoding='utf-8') as source:
            with open(without_multi_line_file_path, 'w', encoding='utf-8') as destination:
                for line in source:
                    destination.write(line)
        add_statement(file_path, "if")
        add_statement(file_path, "else if")
        add_statement(file_path, "else")
        add_statement(file_path, "for")
        with_brackets_file_path = os.path.join(main_parsing_dir,'with_brackets.c')
        with open(file_path, 'r', encoding='utf-8') as source:
            with open(with_brackets_file_path, 'w', encoding='utf-8') as destination:
                for line in source:
                    destination.write(line)
        add_else_to_if(file_path, preprocess_file_path)
        wrap_brace(preprocess_file_path)
        without_function_file_path = os.path.join(main_parsing_dir,'without_function.c')
        with open(preprocess_file_path, 'r', encoding='utf-8') as source:
            with open(without_function_file_path, 'w', encoding='utf-8') as destination:
                for line in source:
                    destination.write(line)
        # No supplementary subfunction calls
        without_subfunction_file_dir = os.path.join(main_parsing_dir, 'without_subfunction_files')
        without_subfunction_file_path = os.path.join(without_subfunction_file_dir, file_name)
        if not os.path.exists(without_subfunction_file_dir):
            os.makedirs(without_subfunction_file_dir)
        with open(preprocess_file_path, 'r', encoding='utf-8') as source:
            with open(without_subfunction_file_path, 'w', encoding='utf-8') as destination:
                for line in source:
                    destination.write(line)
        ...
        with open(preprocess_file_path, 'r') as file:
            ...
            code = add_sub_func_body(lines, main_dissector, final_code, dissect_list, subfunction_call_format, dissect_func_name_list)
        ...
        all_fields = []
        packet_types_all_fields_path = os.path.join(output_dir, (dissect_name.strip('.c')+'-fields.json'))
        for packet in packet_types_fieltered:
            for field in packet:
                if 'field_identifier' in field:
                    if field['field_identifier'] not in all_fields:
                        all_fields.append(field['field_identifier'])
        with open(packet_types_all_fields_path, 'w', encoding='utf-8') as file:
            json.dump(all_fields, file, indent=4, default = custom_encoder)  
...
def main():
        ...
        source_file_dir = "C:/Users/lhs/Desktop/wireshark/epan/dissectors"
        ...
        proto_list, dissect_list, proto_dissect_map = pre_process(source_file_dir, pre_process_dir, proto_files_dir)
        ...
        # The short name and dissector name of the protocol
        proto_name, dissect_name = 'DCCP', 'dccp'
        # proto_name, dissect_name = 'Modbus/TCP', 'mbtcp'
        # proto_name, dissect_name = 'Modbus RTU', 'mbrtu'
        # proto_name, dissect_name = 'DNP 3.0', 'dnp3.tcp'
        # proto_name, dissect_name = 'S7COMM', 's7comm_cotp'
        # proto_name, dissect_name = 'COTP', 'ositp'
        # proto_name, dissect_name = 'TPKT', 'tpkt'

        target_proto_info = []
        target_proto_info = select_proto(proto_list, dissect_list, proto_files_dir, proto_name, dissect_name, target_proto_info)
        ...
        for target_proto in target_proto_info:
            ...
            print("=============================Main-parsing=============================")
            packet_types = main_analyse(file_path, file_name, func_name, preprocess_file_path, main_parsing_dir, truth_field_dir, output_dir, dissect_list, dissect_name)
